...
int init(int argc, char** argv, Params& sim_params)
{
        ...
        if(find_option(argc,argv,"-h") >0 || find_option(argc,argv,"--help")>0)
        {
                ...
                std::cout<<"type -n <uint> to specify number of bodies/particles to simulate (by default n=10). "<<std::endl;
                std::cout<<"type -dim <uint> to specify the euclidean dimension you wish to simulate in (by default dim=3)"<<std::endl;
                std::cout<<"type -sim_dt <float> to specity the time step of the simulation (in sec) (by default sim_dt = 0.025 sec = 25 usec )"
			<<std::endl;
                std::cout<<"type -display_dt <uint> to specity the time step of the (in usec)  (by default display_dt = 25 usec)"<<std::endl;
                std::cout<<"type -temp <float> to specity the temperature factor of the simulation  (by default temp=1)"<<std::endl;
		return 1;
        }
	...
	sim_params.temp = read_float(argc,argv,"-temp",1.0f);
        ...
        sim_params.NUM_BLOCKS =(sim_params.sim_N+THREADS_PER_BLOCK-1)/THREADS_PER_BLOCK;
        sim_params.NUM_THREADS = (sim_params.sim_N+sim_params.NUM_BLOCKS-1)/sim_params.NUM_BLOCKS;
        return 0;
}
