...
int init(int argc, const char** argv, Params& sim_params)
{
        ...
        if(find_option(argc,argv,"-h") >0 || find_option(argc,argv,"--help")>0)
        {
			...
			<<std::endl;
                std::cout<<"type -display_dt <float> to specity the time step of the (in usec)  (by default display_dt = 25 usec)"<<std::endl;
	...
	sim_params.sim_DIM = read_int(argc,argv,"-dim",3);
        ...
        sim_params.display_dt = read_float(argc,argv,"-display_dt",25.0f);
