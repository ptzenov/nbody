...
int init(int argc, char** argv)
	...
	if(find_option(argc,argv,"-h") >0 || find_option(argc,argv,"--help")>0)
	{
		...
		std::cout<<"enter -bdry {0/1} to specify boundary conditions for the problem: 0 for bounce of walls bdry condition and 1 for periodic bdry condition.(by default bdry=0)"<<std::endl;
		return 1;
	...
	bdry = read_int(argc,argv,"-bdry",0);
	if(N < 3)
	{
	...
	if(bdry <0 || bdry >1){
		...
		cout<<" The program parameter -bdry can only have the values 0 and 1"<<endl;
		cout<<"setting -bdry to  its default value 0"<<endl;
		bdry = 0;	
		}
...
__global__ void launch_kernel(particle_t* Particles,GLfloat* dataBuffer_d){
	...
	// simple integrator!
		...
		{	//periodic boundary conditions 
			for(int d=0;d<DIM_d;d++)
						{
							if((Particles[i].p[d])< -size/2 || (Particles[i].p[d]) > size/2)
							{
								Particles[i].p[d] = ((Particles[i].p[d]) <-size/2) ? size/2 : -size/2;
...
__device__ void interract( particle_t * i, particle_t* j)
{
	...
	{	
			...
			for(int d=0;d<DIM_d;d++)
		{
			u1 = i->v[d]; // old velocity of particle i
			u2 = j->v[d]; // old velocity of particle j
			i->v[d]=(u1*(i->m-j->m)+2*j->m*u2)/(i->m+j->m); //update your own velocity!
